...
export default {
    ...
    methods: {
        ...
        async getTaskList() {
            ...
            const { data: data } = await this.$http.get('showimagemessage');
        ...
        async originalfileDownload(task_id) {
            ...
            // });
            const data = await this.$http.get('downloadimage?task_id=' + task_id + '&file_type=0', {
                responseType: 'blob',
            ...
            fileDownload(data.data, 'originalfile_image_' + task_id + '.zip');
        ...
        async generatefileDownload(task_id) {
            ...
            console.log('this.$http.defaults.baseURL1:', this.$http.defaults.baseURL);
            this.$http.defaults.baseURL = 'http://129.211.89.155:8050/';
            const data = await this.$http.get('downloadimage?task_id=' + task_id + '&file_type=1', {
                responseType: 'blob',
                headers: { 'Content-Type': 'application/json; application/octet-stream' }
            });
            this.$http.defaults.baseURL = temp;
            ...
            console.log('data:', data);
            fileDownload(data.data, 'generatefile_image_' + task_id + '.zip');
        },
